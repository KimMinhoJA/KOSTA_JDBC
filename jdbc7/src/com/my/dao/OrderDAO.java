...
//				pstmt.executeUpdate();
	...
	public OrderInfo selectByNo(int order_no) throws NotFoundException {
		...
		try {
			con = MyConnection.getConnection();
			String selectByNoSQL = "SELECT order_no, order_id, order_dt" + ", c.name"
					+ ", order_prod_no, order_quantity" + ", prod_name, prod_price"
					+ " FROM order_info NATURAL JOIN order_line" + " JOIN customer c ON order_info.order_id = c.id"
					+ " JOIN product p ON order_line.order_prod_no = p.prod_no" + " WHERE order_no = ?";
	...
	public List<OrderInfo> selectById(String id) throws NotFoundException {
		...
		try {
			...
			String selectByIdSQL = "SELECT order_no, order_id, order_dt, name, order_prod_no, order_quantity, prod_name, prod_price"
					+ " FROM order_info NATURAL JOIN order_line" + " JOIN customer c ON order_info.order_id = c.id"
					+ " JOIN product p ON order_line.order_prod_no = p.prod_no" + " WHERE order_id = ?";
